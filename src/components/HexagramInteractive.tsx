  ...
  useEffect(() => {
    ...
    function applyHighlight(index: number | null) {
      ...
      rows.forEach((row, j) => {
        ...
        const color = isActive ? (bits[j] === '1' ? '#60a5fa' : '#5090d0') : (bits[j] === '1' ? '#e0e0f0' : '#808098');
        for (let k = 0; k < segs.length; k++) {
          segs[k].style.background = color;
          segs[k].style.boxShadow = isActive && bits[j] === '1' ? '0 0 16px rgba(96,165,250,0.6)' : 'none';
        }
      });
        ...
        {[0, 1, 2, 3, 4, 5].map((i) => (
          ...
          >
            <div className="yao-line-inner" style={{ display: 'flex', justifyContent: 'center', gap: 6 }}>
              {bits[i] === '1' ? (
                <div style={{ width: LINE_W, height: LINE_H, background: '#e0e0f0', borderRadius: 4, transition: 'all 0.2s' }} />
              ) : (
                <>
                  <div style={{ width: LINE_W * 0.42, height: LINE_H, background: '#808098', borderRadius: 4, transition: 'all 0.2s' }} />
                  <div style={{ width: LINE_W * 0.42, height: LINE_H, background: '#808098', borderRadius: 4, transition: 'all 0.2s' }} />
                </>
              )}
            </div>
