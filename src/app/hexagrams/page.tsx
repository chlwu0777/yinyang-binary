  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <h1 style={{ fontSize: 32, fontWeight: 300, marginBottom: 8, color: theme.text }}>{i.hexagrams.title}</h1>
        ...
        <div style={layout.card(theme)}>
          ...
          <div style={{ display: 'flex', gap: 36, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'visible' }}>
              ...
              <div style={{ display: 'flex', gap: 28, marginTop: 20 }}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 28, color: theme.text }}>{trigramInfo[selectedHexagram.binary.slice(3)]?.symbol}</p>
                  <p style={{ fontSize: 14, color: theme.text }}>{trigramInfo[selectedHexagram.binary.slice(3)]?.name}</p>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: 12, color: theme.sub }}>{lang === 'cn' ? '下卦' : 'Lower'}</p>
                  <p style={{ fontSize: 28, color: theme.text }}>{trigramInfo[selectedHexagram.binary.slice(0, 3)]?.symbol}</p>
                  <p style={{ fontSize: 14, color: theme.text }}>{trigramInfo[selectedHexagram.binary.slice(0, 3)]?.name}</p>
                </div>
              </div>
            </div>
            <div style={{ flex: 1, minWidth: 280 }}>
              ...
              <h2 style={{ fontSize: 52, marginTop: 18, marginBottom: 10, color: theme.text }}>{lang === 'cn' ? selectedHexagram.name : selectedHexagram.nameEn}</h2>
              ...
              <div style={{ padding: 18, borderRadius: 12, marginBottom: 18, background: theme.subtleBg, border: `1px solid ${theme.border}` }}>
                ...
                <p style={{ fontSize: 14, marginTop: 6, color: theme.text }}>{lang === 'cn' ? selectedHexagram.desc : selectedHexagram.descEn} · {lang === 'cn' ? selectedHexagram.descEn : selectedHexagram.desc}</p>
              </div>
              <div style={{ padding: 18, borderRadius: 12, marginBottom: 18, background: theme.subtleBg }}>
                ...
                <p style={{ fontSize: 15, color: theme.text }}>{lang === 'cn' ? selectedHexagram.kw : selectedHexagram.kwEn} · {lang === 'cn' ? selectedHexagram.kwEn : selectedHexagram.kw}</p>
              </div>
              <div style={{ padding: 18, borderRadius: 12, background: theme.subtleBg, borderLeft: `4px solid ${theme.border}` }}>
                ...
                <p style={{ fontStyle: 'italic', fontSize: 15, color: theme.text }}>{lang === 'cn' ? selectedHexagram.img : selectedHexagram.imgEn} · {lang === 'cn' ? selectedHexagram.imgEn : selectedHexagram.img}</p>
              </div>
        ...
        <div className="hex-grid">
          {hexagramsData.map((h) => (
            ...
            >
              ...
              <span style={{ fontSize: 15, fontWeight: 500, color: theme.text }}>{lang === 'cn' ? h.name : h.nameEn}</span>
