  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 16 }}>
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        <h1 style={{ fontSize: 32, fontWeight: 300, marginBottom: 8 }}>{i.masters.title}</h1>
        ...
        <div style={layout.card(theme)}>
          ...
          <div style={{ display: 'flex', gap: 36, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 280 }}>
              <h2 style={{ fontSize: 32, marginBottom: 6 }}>{lang === 'cn' ? selectedMaster.name : selectedMaster.nameEn}</h2>
              ...
              <blockquote style={{ margin: '24px 0', padding: 20, borderRadius: 12, fontStyle: 'italic', background: theme.subtleBg, borderLeft: `4px solid ${theme.border}` }}>
                <p style={{ fontSize: 16 }}>{lang === 'cn' ? selectedMaster.quote : selectedMaster.quoteEn}</p>
                <p style={{ fontSize: 14, marginTop: 10, color: theme.sub }}>{lang === 'cn' ? selectedMaster.quoteEn : selectedMaster.quote}</p>
              </blockquote>
              <div style={{ marginBottom: 28 }}>
                <p style={{ lineHeight: 1.9, marginBottom: 16, fontSize: 15 }}>{lang === 'cn' ? selectedMaster.description : selectedMaster.descriptionEn}</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                <div style={{ padding: 18, borderRadius: 12, background: theme.subtleBg }}>
                  ...
                  <p style={{ fontSize: 15 }}>{lang === 'cn' ? selectedMaster.contribution : selectedMaster.contributionEn}</p>
                ...
                <div style={{ padding: 18, borderRadius: 12, background: theme.subtleBg }}>
                  ...
                  <p style={{ fontSize: 15 }}>{lang === 'cn' ? selectedMaster.significance : selectedMaster.significanceEn}</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 18 }}>
          {masters.map((m) => (
            <button type="button" key={m.id} onMouseDown={(e) => { e.preventDefault(); setSelectedMaster(m); }} style={{ ...layout.card(theme), padding: 24, cursor: 'pointer', border: `1px solid ${theme.border}`, textAlign: 'left' as const }}>
              ...
              <p style={{ fontSize: 20, fontWeight: 500, textAlign: 'center' }}>{lang === 'cn' ? m.name : m.nameEn}</p>
